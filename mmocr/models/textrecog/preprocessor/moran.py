...
class MORAN(BasePreprocessor):
    def __init__(self,
        ...
        self.targetH = img_size[0]
        self.targetW = img_size[1]
        ...
        self.cnn = nn.Sequential(
            nn.MaxPool2d(2, 2),
            nn.Conv2d(num_img_channel, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU(True), nn.MaxPool2d(2, 2),
        ...
        self.register_buffer(
            'grid',
            torch.tensor(self.grid_process(maxBatch)).float())
    ...
    def grid_process(self,maxBatch):
    ...
    def forward(self, x, test=None, enhance=0, debug=False):
        ...
        offsets_grid = nn.functional.grid_sample(offsets_pool, grid,padding_mode='border',align_corners=True)
        offsets_grid = offsets_grid.permute(0, 2, 3, 1).contiguous()
        offsets_x = torch.cat([grid_x, grid_y + offsets_grid], 3)
        x_rectified = nn.functional.grid_sample(x, offsets_x,padding_mode='border',align_corners=True)
        ...
        for iteration in range(enhance):
            ...
            offsets_grid += nn.functional.grid_sample(offsets_pool, grid,padding_mode='border',align_corners=True).permute(0, 2, 3, 1).contiguous()
            offsets_x = torch.cat([grid_x, grid_y + offsets_grid], 3)
            x_rectified = nn.functional.grid_sample(x, offsets_x,padding_mode='border',align_corners=True)
