...
class Count_AttnConvertor(BaseConvertor):
    ...
    def str2tensor(self, strings):
        ...
        Returns:
            dict (str: Tensor | list[tensor]):
                tensors (list[Tensor]): [torch.Tensor([<start>,1,2,3,3,4,<end>,<PAD>,<PAD>,<PAD>]),
                                                    torch.Tensor([<start>,5,4,6,3,7,<end>,<PAD>,<PAD>,<PAD>])]
        ...
        for index in indexes:
            # tensor[0] = torch.LongTensor(len(index))
            ...
            # pad_idx = 24
            # pad_target = [24,24,24,24,24,24] -> [1,2,3,3,4,24]
    ...
    def tensor2idx(self, outputs, img_metas=None):
        ...
        for idx in range(batch_size):
            ...
            char_num = max_idx[0].cpu().detach().numpy().astype(int)
            if char_num <=0:
                char_num = 1
            max_value, max_idx = max_value[1:], max_idx[1:]

            str_index, str_score = [], []
            output_index = max_idx.cpu().detach().numpy().tolist()
            output_score = max_value.cpu().detach().numpy().tolist()

            # if char_num > len(output_index):
            #     char_num = len(output_index)

            for i in range(char_num):
                char_index = output_index[i]
                char_score = output_score[i]
