...
class AttentionModule(nn.Module):
    def __init__(self, dim,conv0_k=5,conv0_p=2,conv_sp_k=7,conv_sp_p=9):
        super().__init__()
        self.conv0 = nn.Conv2d(dim, dim, conv0_k, padding=conv0_p, groups=dim)
        self.conv_spatial = nn.Conv2d(dim, dim, conv_sp_k, stride=1, padding=conv_sp_p, groups=dim, dilation=3)
...
class SpatialAttention(nn.Module):
    def __init__(self, d_model,conv0_k,conv0_p,conv_sp_k,conv_sp_p):
        super().__init__()

        self.proj_1 = nn.Conv2d(d_model, d_model, 1)
        self.activation = nn.GELU()
        self.spatial_gating_unit = AttentionModule(d_model,conv0_k,conv0_p,conv_sp_k,conv_sp_p)
...
class Block(nn.Module):
    def __init__(self, dim, mlp_ratio=4., drop=0.,drop_path=0., act_layer=nn.GELU,conv0_k=5,conv0_p=2,conv_sp_k=7,conv_sp_p=9):
        super().__init__()
        self.norm1 = nn.BatchNorm2d(dim)
        self.attn = SpatialAttention(dim,conv0_k,conv0_p,conv_sp_k,conv_sp_p)
...
class VAN(BaseModule):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                mlp_ratios=[4, 4, 4, 4], drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 depths=[3, 4, 6, 3], num_stages=4, flag=False, conv0_k=[5,(3,5),(3,3),(3,3)], conv0_p=[2,(1,2),(1,1),(1,1)],conv_sp_k=[7,(5,7),(3,7),(3,7)],conv_sp_p=[9,(6,9),(3,9),(3,9)]):
        super().__init__()
        if flag == False:
        ...
        for i in range(num_stages):
            patch_embed = OverlapPatchEmbed(img_size=img_size // (2 ** (i-1)) if i== 3 else img_size // (2 ** (i)),
                                            patch_size=3,
            ...
            block = nn.ModuleList([Block(
                dim=embed_dims[i], mlp_ratio=mlp_ratios[i], drop=drop_rate, drop_path=dpr[cur + j],conv0_k=conv0_k[i],conv0_p=conv0_p[i],conv_sp_k=conv_sp_k[i],conv_sp_p=conv_sp_p[i])
