...
from mmocr.models.builder import PREPROCESSOR, BACKBONES
...
class Transformation_Parameter_Estimation(nn.Module):
    ...
    def __init__(self, img_channel, point_channel,num_img_channel,point_size,img_size):
        super().__init__()
        ...
        self.atten = ModuleList([
            DGAB(dim = num_img_channel, point = self.num_fiducial,width = img_size[1], high = img_size[0])
            # DGAB(dim= [16,64], point=self.num_fiducial, width=img_size[1],high=img_size[0])
...
class TPS_PP(BaseModule):
    ...
    def __init__(self,
        ...
        self.MSFA = Multi_Scale_Fearue_Aggregation(self.num_img_channel,self.point_size,p_stride, num_map = 3)
        self.TPE = Transformation_Parameter_Estimation(self.point_channel,self.img_channel,self.num_img_channel,self.point_size,self.img_size)
        ...
        if self.type=='ResNet45v2':
            ...
            self.up_sample = nn.Upsample(
                ...
                mode='nearest',
                align_corners=None)
            self.down_feat = ConvModule(3*self.img_channel, self.img_channel, kernel_size=1, stride=1, )
        elif self.type == 'ResNet45':
    ...
    def forward(self, batch_img, outs):
        """
        Args:
        ...
        """
        feat_cat = batch_img
        if outs != None and self.type=='ResNet45':
