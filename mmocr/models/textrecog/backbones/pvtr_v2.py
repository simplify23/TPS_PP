...
class SPTR(BaseModule):
    ...
    def __init__(self,
        ...
        if patch_merging is not None:
            ...
            #                              types=patch_merging)  # ConvBNLayer(embed_dim[0], embed_dim[1], kernel_size=3, stride=[2, 1], sub_norm=sub_norm)
            # HW = [self.HW[0] // 2, self.HW[1]]
            self.sub_sample3 = SubSample(embed_dim[2], embed_dim[3], sub_norm=sub_norm, stride=[2, 2],
                                         types=patch_merging)  # ConvBNLayer(embed_dim[0], embed_dim[1], kernel_size=3, stride=[2, 1], sub_norm=sub_norm)
            HW = [self.HW[0] // 8, self.HW[1] // 8]
        else:
            HW = self.HW
        ...
        self.blocks4 = nn.ModuleList([
            Block_unit(
                ...
                drop_path=dpr[depth[0] + depth[1]:][i],
                norm_layer=norm_layer,
                **ex_arg[2]) for i in range(depth[3])
        ])
        # if patch_merging is not None:
        #     # self.sub_sample1 = SubSample(embed_dim[0], embed_dim[1], sub_norm=sub_norm, stride=[2, 1],
        #     #                              types=patch_merging)  # ConvBNLayer(embed_dim[0], embed_dim[1], kernel_size=3, stride=[2, 1], sub_norm=sub_norm)
    ...
    def forward_features(self, x, tpsnet=None):
        ...
        if self.patch_merging is not None:
            x = self.sub_sample1_0(x.transpose(1, 2).reshape(B, self.embed_dim[0], self.HW[0], self.HW[1]))

        if tpsnet != None:
            # draw_feature_map(x)
            x = rearrange(x, 'b (h w) c -> b c h w', h=16, w=64)
            out.append(x)
            outputs = tpsnet(x, epoch=5, outs=out)
            ...
            # x = self.up_linear(x)
        x = self.sub_sample1_1(x.transpose(1, 2).reshape(B, self.embed_dim[0], self.HW[0]//2, self.HW[1]//2))
        ...
        if self.patch_merging is not None:
            # x = self.sub_sample2(x.transpose(1, 2).reshape(B, self.embed_dim[1], self.HW[0] // 2, self.HW[1]))
            x = self.sub_sample3(x.transpose(1, 2).reshape(B, self.embed_dim[2], self.HW[0] // 4, self.HW[1] // 4))
    ...
    def forward(self, x, tpsnet=None,test=None):
        x = self.forward_features(x, tpsnet=tpsnet)
...
# def DeiT_tiny_patch4_32_100(pretrained=False, use_ssld=False, **kwargs):
...
# sptrnet = SPTRNet(img_size=[32, 100],
#                   embed_dim=[64, 128, 256],
#                   depth=[3, 6, 3],
#                   num_heads=[2, 4, 8],
#                   mixer=['Conv', 'Conv', 'Conv', 'Conv', 'Conv', 'Conv', 'Global', 'Global', 'Global', 'Global',
#                          'Global', 'Global'],
...
# x = np.random.uniform(0, 1, [1, 3, 32, 100]).astype(np.float32)
#
# x = torch.tensor(x)
#
# # params_info = paddle.summary(sptrnet, (1, 3, 32, 100))
# # print(params_info)
#
# # model_info = paddle.flops(sptrnet, (1, 3, 32, 100), print_detail=False)
# # print(model_info)
#
# y = sptrnet(x)
# print(y.shape)
#
# print(OCR_VIT.flops() / 1e6)
#
# for i in range(10):
#     inputs = np.random.rand(1, 3, 32, 100).astype(np.float32)
