...
from mmocr.models.textrecog.backbones.tps import U_TPSnet
from mmocr.models.textrecog.layers import BasicBlock


@BACKBONES.register_module()
class ResNetABI_v3(BaseModule):
    ...
    def __init__(self,
        ...
        self._make_point_stem_layer(stem_channels, stem_channels*2)
        ...
        for i, num_blocks in enumerate(arch_settings):
            ...
            tps = U_TPSnet(point_size=(p_h // p_stride, p_w // p_stride),
                           p_stride = p_stride,
                           img_size=(h // stride, w // stride),
                           rectified_img_size=(h // stride, w // stride),
                           num_img_channel=planes)
            # self.inplanes if before resnet
            ...
            p_h, p_w = p_h // p_stride, p_w // p_stride
    ...
    def _make_point_stem_layer(self, in_channels, stem_channels):
        self.p_conv = nn.Sequential(
            ...
            nn.BatchNorm2d(stem_channels),
            nn.ReLU(True),
            nn.MaxPool2d(2, 2),  # batch_size x 64 x img_height/4 x img_width/4
            # nn.Conv2d(
            #     stem_channels, stem_channels, kernel_size=3, stride=1, padding=1),
            # nn.BatchNorm2d(stem_channels),
            # nn.ReLU(True),
            # nn.MaxPool2d(2, 2),  # batch_size x 64 x img_height/8 x img_width/8
        )
